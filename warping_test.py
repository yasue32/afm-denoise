...
def main(load_filepath, sub_dir, save_filepath="afm_dataset", write_all_index=False, file_name="{:05}.png", 
    ...
    for i in range(n_set):
        ...
        for j, patch in enumerate(patches):
            for img in patch:
                #print(img.shape)
                flow = get_flow(patch[0], img)
                for y in range(len(img)):
                    for x in range(len(img[0])):
                        print(img[y][x])
                        print(patch[0].shape)
                        img[y][x] = patch[0][y-flow[y,x,0]][x-flow[y,x,1]]
                ...
                plt.imshow(img)
                plt.savefig(save_filepath+"/"+str(j)+".png")
                plt.show()
                plt.clf()
                plt.close()


main("orig_img", "211112", save_filepath="warping", kernel_size=7, n_patch=1,write_all_index=False)