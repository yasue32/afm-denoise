...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return input, target, neigbors, flows, bicubic, good_img_bools, images
    ...
    def _data_augmentation(self, images, flip_h=True, rot=True, gamma=True):
        ...
        if gamma:
            gamma_val = (random.random()/8+0.88)
            for i in range(length): 
...
if __name__ == '__main__':
    ...
    dataset[0] = TrainDataset('ext_clean_dataset_per_sequence', 7, train=True, noise_flow_type="p", optical_flow="p", warping=True)
    dataloader[0] = DataLoader(dataset[0], batch_size=2, shuffle=False, num_workers=1)

    dataset[1] = TrainDataset('ext_clean_dataset_per_sequence', 7, train=True, noise_flow_type="p", optical_flow="p", warping=True)
    dataloader[1] = DataLoader(dataset[1], batch_size=2, shuffle=False, num_workers=1)


    save_dir = 'threshold_dicision'
    ...
    i = 0
    for data0,data1 in zip(dataloader[0], dataloader[1]):
        input, target, neigbor, flows, bicubic, good_img_bools0, images = data0[0], data0[1], data0[2], data0[3], data0[4], data0[5], data0[6]
        input, target, neigbor, flows, bicubic, good_img_bools1, images = data1[0], data1[1], data1[2], data1[3], data1[4], data1[5], data1[6]
        #print(i, input.shape[0])

        print(neigbor.shape)
        for b in range(input.shape[0]):
            images_cat = (np.hstack([im for im in images[b]]))
            # save_torch_img(images_cat, os.path.join(
                # save_dir, str(i).zfill(4) + '_clean_dirty.png'))
            plt.figure(figsize=(8,8))
            plt.imshow(images_cat)
            # print(good_img_bools0, good_img_bools1)
            plt.title("pyflow: "+",".join([str(boo.item()) for boo in good_img_bools0[b]])+"   SIFT: "+",".join([str(boo.item()) for boo in good_img_bools1[b]])) 
            plt.savefig(os.path.join(save_dir, str(i).zfill(4) + '_comparison.png'), bbox_inches='tight')
            plt.clf()
            plt.close()

            neigbor_cp = [n for n in neigbor[b]]
            # print([n[b].shape for n in neigbor] , target[b].shape, input[b].shape)
            neigbor_cat = torch.cat([target[b]] + [input[b]] + neigbor_cp , 2)
            save_torch_img(neigbor_cat, os.path.join(
                save_dir, "_" + str(i).zfill(4) + '_clean_dirty.png'))
            i += 1
