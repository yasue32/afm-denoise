        ...
        for i in seq:
            ...
            if os.path.exists(file_name):
                if upscale_only:
                    ...
                    temp = modcrop(Image.open(filepath[0:char_len - 7] + '{0:03d}'.format(index) + '.png').convert(
                        'RGB'), scale).resize((int(target.size[0] / scale), int(target.size[1] / scale)), Image.BICUBIC)
                neigbor.append(temp)
            else:
    ...
    if other_dataset:
        ...
        if shuffle:  # add by shinjo 1120
            ...
            if upscale_only:
                ...
                neigbor = [modcrop(Image.open(nfilepath).convert('RGB'), scale).resize(
                    (int(target.size[0] / scale), int(target.size[1] / scale)), Image.BICUBIC) for nfilepath in neigbor_index]
            ...
            seq = [x for x in range(1, nFrames)]
            # random.shuffle(seq) #if random sequence
            for i in seq:
    ...
    if other_dataset:
        ...
        if shuffle:  # add by shinjo 1120
            ...
            if upscale_only:
                ...
                neigbor = [modcrop(Image.fromarray(np_load_int(nfilepath)), scale).resize(
                    (int(target.size[0] / scale), int(target.size[1] / scale)), Image.BICUBIC) for nfilepath in neigbor_index]
            neigbor.extend([input for i in range(nFrames - 1 - len(neigbor))])
            neigbor_index.extend([filepath for i in range(nFrames - 1 - len(neigbor))])
        else:
            ...
            for i in seq:
                ...
                if os.path.exists(file_name1):
                    if upscale_only:
                        ...
                        # temp = modcrop(Image.open(file_name1).convert('RGB'), scale).resize(
                        # (int(target.size[0]/scale),int(target.size[1]/scale)), Image.BICUBIC)
                        temp = modcrop(Image.fromarray(np_load_int(file_name1))).resize(
                            (int(target.size[0] / scale), int(target.size[1] / scale)), Image.BICUBIC)
                    neigbor.append(temp)
                    neigbor_index.append(file_name1)
                else:
...
def get_gt(imgs, nFrames):
    ...
    neigbor_list = sorted(neigbor_list)
...
class DatasetFromFolder(data.Dataset):
    def __init__(self, image_dir, nFrames, upscale_factor, data_augmentation, file_list, other_dataset, patch_size, future_frame, shuffle,
                 transform=None, upscale_only=True, warping=False, alignment=False, depth_img=False, optical_flow="s", random_crop=False):
    ...
    def __getitem__(self, index):
        if self.random_crop == 0:
            if self.future_frame:
                if self.depth_img is False:
                    target, input, neigbor, input_filepath, neigbor_filepath = load_img_future(
                        self.image_filenames[index], self.nFrames, self.upscale_factor, self.other_dataset, self.shuffle, self.upscale_only)
                else:
                    target, input, neigbor, input_filepath, neigbor_filepath = load_img_future_depth(
                        self.image_filenames[index], self.nFrames, self.upscale_factor, self.other_dataset, self.shuffle, self.upscale_only)
            else:
...
class DatasetFromFolderTest(data.Dataset):
    def __init__(self, image_dir, nFrames, upscale_factor, file_list, other_dataset, future_frame, transform=None, upscale_only=True, warping=False,
                 alignment=False, depth_img=False, optical_flow="s"):
    ...
    def __getitem__(self, index):
        # if self.future_frame:
        #     target, input, neigbor, input_filepath, neigbor_filepath = load_img_future(
        #       self.image_filenames[index], self.nFrames, self.upscale_factor, self.other_dataset, False, self.upscale_only)
        # else:
